...
class Profile_view:
    ...
    def profile_window(self, profile, s, profile_button):
        s.iconbitmap("images\\profile.ico")
...
def forgot_password(email, *OTP):
    try:
        ...
        otp = f"Hey {OTP[2]}! Your OTP for your ONE-PASS manager is {OTP[0]}.Please use this to verify your email"
        msg = "Subject: {}\n\n{}".format(SUBJECT, otp)
...
def window_after(username, hash_password, password_new, *window):
    ...
    def note_pad_sec():
        ...
        button["state"] = NORMAL
        profile_button["state"] = NORMAL
        root.iconbitmap("images\\_notes.ico")
        ...
        if __name__ == "__main__":
            ...
            # Basic tkinter setup
            root.geometry("1300x700")
            root.iconbitmap(False, "images\\_notes.ico")
            root.title("Untitled - Notepad")
            # Add TextArea
            root.resizable(0, 0)
    ...
    notes_img = tk_image.PhotoImage(image.open("images\\_notes.png"))
    mainarea = Frame(root, bg="#292A2D", width=500, height=500)
    mainarea.pack(expand=True, fill="both", side="right")
    new_button = tk_image.PhotoImage(image.open("images\\_new_but.jpg"))
    button = Button(
        sidebar,
        image=new_button,
        text='Passwords',
...
class Password_display(Frame):
    ...
    def show_account(self, account_name):
        ...
        #     username, hashed_password, window, object)
        ...
        copy_but_password = Button(new_s, text="Copy Password", bg="#292A2D", fg="white", font=(
            "Cascadia Mono SemiBold", 9), command=lambda: copy(password))
        copy_but_username = Button(new_s, text="Copy Username", bg="#292A2D", fg="white", font=(
            "Cascadia Mono SemiBold", 9), command=lambda: copy(username))
